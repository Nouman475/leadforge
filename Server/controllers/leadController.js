...
const leadSchema = Joi.object({
  ...
  status: Joi.string().valid('new', 'contacted', 'qualified', 'proposal', 'closed').optional(),
...
class LeadController {
  ...
  async createLead(req, res) {
    try {
      ...
      const existingLead = await Lead.findOne({ where: { email: value.email } });
      if (existingLead) {
  ...
  async bulkCreateLeads(req, res) {
    try {
      ...
      for (let i = 0; i < leads.length; i++) {
        const { error, value } = leadSchema.validate(leads[i]);
        if (error) {
          errors.push({
            index: i,
      ...
      if (errors.length > 0) {
        return res.status(400).json({
          success: false,
          error: 'Validation failed for some leads',
          details: errors
        });
      }

      const createdLeads = await Lead.bulkCreate(validatedLeads, {
