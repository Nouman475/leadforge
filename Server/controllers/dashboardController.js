const { Lead, EmailCampaign, EmailHistory, EmailTemplate } = require('../models');
...
class DashboardController {
  ...
  async getRecentActivity() {
    ...
    const recentEmails = await EmailHistory.findAll({
      ...
      attributes: ['id', 'recipient_name', 'subject', 'status', 'sent_at', 'created_at']
    });
  ...
  async getGrowthMetrics(startDate) {
    ...
    const previousPeriodStart = moment(startDate).subtract(
      moment().diff(startDate, 'days'), 'days'
    ).toDate();
      ...
      res.json({
        ...
        data: {
          ...
          conversions: Object.fromEntries(
            Object.entries(conversions).map(([key, value]) => [key, parseFloat(value)])
          ),
          total,
          period: parseInt(period)
        }
  ...
  async getEmailPerformanceOverTime(req, res) {
    try {
      ...
      const dailyStats = await EmailHistory.findAll({
        attributes: [
          [EmailHistory.sequelize.fn('DATE', EmailHistory.sequelize.col('sent_at')), 'date'],
          'status',
          [EmailHistory.sequelize.fn('COUNT', EmailHistory.sequelize.col('id')), 'count']
        ],
        where: { 
          sent_at: { [Op.gte]: startDate },
        ...
        group: [
          EmailHistory.sequelize.fn('DATE', EmailHistory.sequelize.col('sent_at')),
          'status'
        ],
        order: [[EmailHistory.sequelize.fn('DATE', EmailHistory.sequelize.col('sent_at')), 'ASC']]
      });
  ...
  async getTemplateUsage(req, res) {
    try {
      ...
      const categoryUsage = await EmailTemplate.findAll({
        attributes: [
          'category',
          [EmailTemplate.sequelize.fn('SUM', EmailTemplate.sequelize.col('usage_count')), 'total_usage']
        ],
        group: ['category'],
        order: [[EmailTemplate.sequelize.fn('SUM', EmailTemplate.sequelize.col('usage_count')), 'DESC']]
      });
